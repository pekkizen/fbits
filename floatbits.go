...
// UlpDiff returns the ulp of x by subtracting adjacent floats.
// Special cases:
// Ulp(+/-Inf) = NaN ?
// Ulp(NaN)    = NaN
// 
func UlpDiff(x float64) float64 {
	u := math.Float64bits(x) & posInf 
	return math.Float64frombits(u + 1) - math.Float64frombits(u)
}

// UlpBits returns the ulp of x without floating-point calculations.
// This is ~20% slower than the other two.
// Special cases:
// Ulp(+/-Inf) = +Inf (as abs(x))
// Ulp(NaN)    = NaN  (as abs(x))
// 
func UlpBits(x float64) float64 {
...
func LogUlp(x float64) (exp int) {
	u := int(math.Float64bits(x) &^ signbit >> 52)
	exp = u - (1023 + 52)
	if u == 0 {
		exp = -1074
	} 
...
// 00000 (floatbits.go:173)	MOVQ	"".x+8(SP), AX
// 00005 (floatbits.go:175)	MOVQ	AX, CX
// 00008 (floatbits.go:175)	SHLQ	$12, AX
// 00012 (floatbits.go:177)	LEAQ	-1(AX), DX
// 00016 (floatbits.go:177)	TESTQ	AX, DX
// 00019 (floatbits.go:177)	JNE	56                  Jump out if s&(s-1)==0 is false
// 00021 (floatbits.go:177)	TESTQ	AX, AX
// 00024 (floatbits.go:177)	SETHI	AL
// 00027 (floatbits.go:174)	SHRQ	$52, CX
// 00031 (floatbits.go:177)	TESTQ	CX, CX
// 00034 (floatbits.go:177)	SETHI	DL
// 00037 (floatbits.go:177)	CMPB	AL, DL
// 00039 (floatbits.go:177)	JEQ	56
// 00041 (floatbits.go:177)	CMPQ	CX, $2047
// 00048 (floatbits.go:177)	SETCS	AL
// 00051 (floatbits.go:177)	MOVB	AL, "".~r1+16(SP)
// 00055 (floatbits.go:177)	RET
// 00056 (floatbits.go:177)	XORL	AX, AX
// 00058 (floatbits.go:177)	JMP	51
